...
export const OrangeDirectorySection: React.FC = () => {
  ...
  return (
    <section
      id="directory"
      className="relative min-h-screen h-screen bg-[#0F1926] text-[#5E656E] flex flex-col justify-between overflow-hidden select-none scroll-mt-0 pb-28 sm:pb-32 border-t border-[#2A2A2B]"
    >
      {/* TOP UTILITY HEADER STRIP */}
      <div className="bg-[#0C1015] text-[#5E656E] py-3.5 sm:py-4 px-6 sm:px-12 flex items-center justify-between border-b border-[#2A2A2B] shrink-0 z-20">
        <div>
          <a
            href="mailto:Info@tanushfitness.com"
            className="font-mono text-xs sm:text-sm text-[#D0CFCA] hover:text-[#5E656E] underline tracking-wider transition-colors"
          >
        ...
        <div className="flex items-center gap-5 sm:gap-6">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-[#D0CFCA] hover:text-[#5E656E] transition-colors"
            aria-label="LinkedIn"
          >
      ...
      <div className="relative flex-1 max-w-7xl mx-auto w-full px-6 sm:px-12 my-auto flex items-center overflow-hidden">
        ...
        <div className="space-y-4 sm:space-y-5 md:space-y-6 z-10 w-full max-w-3xl">
          {DIRECTORY_ITEMS.map((item) => (
            ...
            >
              <span className="font-mono text-xs sm:text-sm md:text-base text-[#D0CFCA] group-hover:text-[#5E656E] font-bold tracking-widest shrink-0 transition-colors">
                {item.num}
              </span>

              <div className="relative flex-1">
                <span className="font-satoshi text-2xl sm:text-3xl md:text-4xl lg:text-[40px] font-black uppercase text-[#5E656E] group-hover:text-[#D0CFCA] group-hover:translate-x-3 transition-all duration-300 block leading-tight">
                  {item.title}
                </span>
                <div className="h-[1px] bg-[#2A2A2B] w-full mt-2 group-hover:bg-[#5E656E] transition-colors duration-300" />
              </div>
            </div>
          ))}
        </div>
